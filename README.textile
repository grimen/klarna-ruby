...
Sweden (SE), Norway (NO), Denmark (DK), Finland (FI), and Germany (DE).
...
<pre>
  ...
  Klarna.setup do |config|
    config.store_id = 123
    config.store_secret = 'AbCdEfGh123'
    config.p_classes_class = ::Klarna::API::PClasses
  end
...
Endpoint: @http://beta-test.kreditor.se:4567@
Method: @get_addresses@
Params: @["1.0","ruby/xmlrpc","430415-8399",2,"AY2qnVNi8ccTsuDw2XNovQ==",2,2]@

h2. References

* "Klarna API Documentation":http://integration.klarna.com/api-functions/overview
...
Copyright (c) "Jonas Grimfelt":http://github.com/grimen for "Socio-T":http://socio-t.com
